-----------------------------------
-- SOME/IP Dissector
-- Copyright 2020 ATMES GmbH

-- Version 1.3    - Add decoding of SOME/IP Transport Protocol
...
local pTp = Proto("someiptp","someiptp")
local f_tpMoreFlag     = ProtoField.uint8("someiptp.tpMoreFlag","More Segment Flag",base.HEX)
local f_tpOffset = ProtoField.uint16("someiptp.tpOffset","Offset",base.HEX)
pTp.fields = {f_tpMoreFlag, f_tpOffset}
...
local function getTp(buf, pkt, root)

	subtree = root:add("Transport Protocol")
	
	local offset_bytes =  bit.rshift(buf:range(1,3):uint(), 4)
	local moreflag_byte = buf:range(3,1)
	
	mf =  moreflag_byte:bitfield(7,1)

	subtree:add(f_tpMoreFlag, mf)	
	subtree:add(f_tpOffset, offset_bytes)
	
end -- of function getTp(buf, pkt, root)
...
function pSomeIP.dissector(buf, pkt, root)
	...
	while (copyStartByte < buf:reported_length_remaining()) do
		if (buf:reported_length_remaining() >= sizeSomeIpHeader) then
			...
			-- Transport Protocol
			---------------------	
			local tpFlag = buf(copyStartByte + 14,1):bitfield(2,1)
			if tpFlag == 1 then
				pkt.cols.protocol = "SOME/IP Transport Protocol"
				pkt.cols.info = "SOME/IP Transport Protocol"
				getTp(buf(copyStartByte + 16):tvb(), pkt, subtree)		
			end
